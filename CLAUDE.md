...
> 適用於任何階段的專案（原型、開發中、已上線都算），因為放寬的只有「不傷錢、不傷資料」的問題。
...
- **程式碼的審查→修正→再審迴圈**（model-dispatch §6「修正後再審」那條）：
  ...
  - **其他 H**（非風險類但會產生錯誤結果）：當場修。計數單位是追蹤檔（`PLAN.md`／`TODO.md`，
    以「本專案對照」寫的為準）的一個功能項；沒有功能項清單時，使用者的一次請求算一個功能項。
    整個功能最多 2 輪審查，修正批不重新計數。第 2 輪仍有 H：照樣修，但不自動開第 3 輪；
    回報列出這些 H、修法，並標「未經再審」，由使用者決定要不要再審。
  - **其他 M**：不修，記進追蹤檔的「延後的 M」清單；功能項可以打勾，但回報要寫出本批延後了幾條 M。
    使用者要求、或上線／交付前，把清單列給使用者排優先序。
  - H 修到一半的殘留仍是 H；本功能範圍內的既有問題不因「不在本次 diff」就排除
    （範圍外的照手術刀原則只回報，不擋收工）。L 不擋收工（照 §6）。
  - 不適用（照全域規則）：不可逆操作／刪資料／對外發布的第二意見、掃描類覆核、制度檔一致性檢查。
- **瀏覽器驗證**：修正 agent 只驗自己修的那條，不巡全畫面。每個功能項收尾時，派一個 `opus`
...
- **收工量測**：每個功能項收尾跑
  ...
  `--until 2026-09-23` 量，同一把尺比，不要拿人工數字比。審查輪數腳本算不出來，從審查紀錄人工數。
  **跨專案參考基準**（2026-09-23 Accounting 原型，未試行）：5 輪覆核；工具呼叫 2,202 次，其中瀏覽器 941 次
  （43%）；fix 類 subagent 佔 87% token、review 類 13%（腳本 token 去重後重量）；主對話約 8.8 小時；撞到一次額度上限。
  （fix／review 佔比是腳本依 agent 描述分類的結果，會把兩個覆核 agent 算成 fix；人工重分是 79%／21%。）
